...
class adsControler {
    async findAddAll(req, res) {
        try {
            ...
            if (!allUserAds) res.status(200).json({
    ...
    async addAds(req, res) {
        try {
            ...
            if (title && text && price && phone && category) {
                await Ads.create({
                    userId: id, title: title, text: text, price: price, phone: phone, category: category,
                })
    ...
    async updateAds(req, res) {
        try {
            ...
            const {title, text, price, phone, category} = req.body;
            await Ads.update({
                title: title, text: text, price: price, phone: phone, category: category,
            }, {
                where: {
                    userId: id,
                }
